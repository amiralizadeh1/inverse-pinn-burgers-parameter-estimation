...
# Define paths
base_path = r"d:\PhD\Implementation\PINN\TensorFlow\Burgers Equation\Revision\performance_vs_N_l"
output_path = base_path

# Get all N_l folders
n_l_folders = sorted([d for d in os.listdir(base_path) if os.path.isdir(os.path.join(base_path, d)) and d.startswith('N_l_')])
n_l_values = [int(folder.split('_')[2]) for folder in n_l_folders]

# Read data from all N_l values
data = {}
for folder_name, n_l in zip(n_l_folders, n_l_values):
...
def extract_fold_data(data, metric_prefix):
    """Extract data for all folds of a given metric across all N_l values"""
    fold_data = {}
    for n_l in sorted(data.keys()):
        df = data[n_l]
        fold_values = []
        for fold in range(3):
        ...
        fold_data[n_l] = np.array(fold_values)
    return fold_data
...
def create_comparison_plot(fold_data, metric_name, ylabel, output_file):
    """Create a comparison plot with mean and std for each N_l, with inset for last 10 iterations"""
    fig, ax = plt.subplots(figsize=(12, 7))
    
    n_l_list = sorted(fold_data.keys())
    ...
    for idx, n_l in enumerate(n_l_list):
        ...
        # Plot mean line
        ax.plot(iterations, mean_vals, label=f"N_l {n_l}", 
                color=colors[idx % len(colors)], linewidth=2)
        
        # Add std shadow with geometric pattern
        ax.fill_between(iterations, mean_vals - std_vals, mean_vals + std_vals,
                        alpha=0.3, color=light_colors[idx % len(light_colors)],
                        hatch=hatches[idx % len(hatches)], edgecolor='none')
    ...
    for idx, n_l in enumerate(n_l_list):
        ...
        # Plot in inset
        axins.plot(last_10_iter, last_10_mean, color=colors[idx % len(colors)], linewidth=2)
...
create_comparison_plot(error_data, "Error", "Error", 
                      os.path.join(output_path, "error_comparison_all_N_l.png"))

create_comparison_plot(lambda_data, "Lambda", "Lambda Value", 
                      os.path.join(output_path, "lambda_comparison_all_N_l.png"))

create_comparison_plot(loss_test_data, "Test Loss", "Test Loss", 
                      os.path.join(output_path, "loss_test_comparison_all_N_l.png"))

create_comparison_plot(loss_train_data, "Training Loss", "Training Loss", 
                      os.path.join(output_path, "loss_train_comparison_all_N_l.png"))
