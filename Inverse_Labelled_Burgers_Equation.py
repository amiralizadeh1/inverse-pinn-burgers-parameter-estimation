...
N_f = 10000
N_l = 1000
layers = np.array([2,10,10,10,10,1])
initial_lambda2 = 0.01
...
# Data Prep
newFolder = os.path.join(os.getcwd(), 'Revision', 'general')
...
class Sequentialmodel(tf.Module):
    ...
    # for train_idx_u, test_idx_u in kf.split(X_u_all):
    #     for train_idx_l, test_idx_l in kf.split(X_labelled_all):
...
if __name__ == "__main__":
    ...
    loss_test_array = np.array(loss_test_histories)
    mean_loss_test = np.mean(loss_test_array, axis=0)
    std_loss_test = np.std(loss_test_array, axis=0)
    ...
    ax.set_ylabel('Loss')
    ax.legend()
    plt.savefig(os.path.join(newFolder, 'loss_train_all_folds.png'))
    plt.close()
    ...
    ax.set_ylabel('Loss')
    ax.legend()
    plt.savefig(os.path.join(newFolder, 'loss_test_all_folds.png'))
    plt.close()
    
    lambda_array = np.array(lambda_histories)
    mean_lambda = np.mean(lambda_array, axis=0)
    std_lambda = np.std(lambda_array, axis=0)
    ...
    ax.axhline(y=0.005/np.pi, color='red', linestyle='--', linewidth=2, label='True Value ($0.005/\\pi$)')
    ax.set_xlabel('Iteration')
    ax.set_ylabel('Lambda Value')
    ax.legend()
    
    axins = inset_axes(ax, width="50%", height="50%", loc='center')
    last_10_mean = mean_lambda[-10:]
    last_10_std = std_lambda[-10:]
    last_10_iter = np.arange(len(mean_lambda) - 10, len(mean_lambda))
    axins.plot(last_10_iter, last_10_mean, linewidth=2, color='tab:blue')
    axins.fill_between(last_10_iter, last_10_mean - last_10_std, last_10_mean + last_10_std, alpha=0.3, color='tab:blue')
    axins.axhline(y=0.001/np.pi, color='red', linestyle='--', linewidth=2)
    
    plt.savefig(os.path.join(newFolder, 'lambda_all_folds.png'))
    plt.close()
    
    error_array = np.array(error_histories)
    ...
    ax.legend()
    plt.savefig(os.path.join(newFolder, 'error_all_folds.png'))
    plt.close()
    
    with open(os.path.join(newFolder, 'results.txt'), 'w') as f:
        ...
        f.write(f'Simulation Time: {hours}h {minutes}m {seconds}s\n')
        f.write(f'Network Architecture: {layers}\n')
        f.write(f'Max Iterations: {maxiter}\n')
        f.write(f'Number of Folds: {fold}\n\n')
        f.write('=== Final Values ===\n')
        f.write(f'Final Mean Training Loss: {mean_loss[-1]:.6e}\n')
        f.write(f'Final Mean Test Loss: {mean_loss_test[-1]:.6e}\n')
        f.write(f'Final Mean Lambda: {mean_lambda[-1]:.6e}\n')
        f.write(f'Final Mean Error: {mean_error[-1]:.6e}\n\n')
        f.write('=== Performance Metrics ===\n')
        f.write(f'Min Training Loss: {np.min(mean_loss):.6e}\n')
        f.write(f'Min Test Loss: {np.min(mean_loss_test):.6e}\n')
        f.write(f'Min Error: {np.min(mean_error):.6e}\n')
        f.write(f'Mean Lambda (avg last 100): {np.mean(mean_lambda[-100:]):.6e}\n')
    
    # Create DataFrame for Excel export
    data_dict = {'iterations': iterations}
    
    for i in range(fold):
        data_dict[f'error_fold_{i}'] = error_histories[i]
        data_dict[f'lambda_fold_{i}'] = lambda_histories[i]
        data_dict[f'loss_test_fold_{i}'] = loss_test_histories[i]
        data_dict[f'loss_train_fold_{i}'] = loss_histories[i]
    ...
    df.to_excel(os.path.join(newFolder, 'results.xlsx'), index=False)
