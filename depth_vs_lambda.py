"""
Network-depth sensitivity study for inverse PINN parameter recovery.
...
The aim is to study how neural-network depth affects recovery of the unknown
physical parameter lambda_2. Several fully connected architectures with the
same hidden-layer width but different numbers of hidden layers are defined,
and one selected depth case is trained in this script.
...
across different network-depth cases.
...
newFolder = os.path.join(baseFolder, 'depth_vs_performance')
...
if __name__ == "__main__":
    ...
    depth_idx = 2
    layers = network_architectures[depth_idx]
    depth_start = time.time()
    depth_folder = os.path.join(newFolder, f'depth_{depth_idx}_layers_{len(layers)-2}')
    if not os.path.exists(depth_folder): os.makedirs(depth_folder)
    ...
    for (train_idx_u, test_idx_u), (train_idx_l, test_idx_l) in zip(kf.split(X_u_all), kf.split(X_labelled_all)):
        ...
        print(f'✅ Fold {fold} finished (depth={len(layers)-2})')
    ...
    df.to_excel(os.path.join(depth_folder, 'results.xlsx'), index=False)
    ...
    ax.set_ylabel('Loss')
    ax.legend()
    plt.savefig(os.path.join(depth_folder, 'loss_train_all_folds.png'))
    plt.close()
    ...
    ax.set_ylabel('Loss')
    ax.legend()
    plt.savefig(os.path.join(depth_folder, 'loss_test_all_folds.png'))
    plt.close()
    ...
    ax.legend()
    plt.savefig(os.path.join(depth_folder, 'lambda_all_folds.png'))
    plt.close()
    ...
    ax.legend()
    plt.savefig(os.path.join(depth_folder, 'error_all_folds.png'))
    plt.close()
    
    depth_elapsed = time.time() - depth_start
    print(f'✅ Depth {len(layers)-2} finished ({depth_elapsed:.2f}s)')
    print(f'Results saved to {depth_folder}')
    sys.exit(0)
