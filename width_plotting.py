...
# Define paths
base_path = r"d:\PhD\Implementation\PINN\TensorFlow\Burgers Equation\Revision\width_vs_performance"
output_path = base_path

# Width configurations
widths = [
    ("width_0_neurons_5", 5),
    ("width_1_neurons_10", 10),
    ("width_2_neurons_15", 15),
    ("width_3_neurons_25", 25)
]

# Read data from all widths
data = {}
for folder_name, width in widths:
...
def extract_fold_data(data, metric_prefix):
    """Extract data for all folds of a given metric across all widths"""
    fold_data = {}
    for width in sorted(data.keys()):
        ...
        fold_data[width] = np.array(fold_values)
    return fold_data
...
def create_comparison_plot(fold_data, metric_name, ylabel, output_file):
    """Create a comparison plot with mean and std for each width, with inset for last 100 iterations"""
    fig, ax = plt.subplots(figsize=(12, 7))
    
    widths_list = sorted(fold_data.keys())
    ...
    for idx, width in enumerate(widths_list):
        ...
        # Plot mean line
        ax.plot(iterations, mean_vals, label=f"Width {width}", 
...
create_comparison_plot(error_data, "Error", "Error", 
                      os.path.join(output_path, "error_comparison_all_widths.png"))

create_comparison_plot(lambda_data, "Lambda", "Lambda Value", 
                      os.path.join(output_path, "lambda_comparison_all_widths.png"))

create_comparison_plot(loss_test_data, "Test Loss", "Test Loss", 
                      os.path.join(output_path, "loss_test_comparison_all_widths.png"))

create_comparison_plot(loss_train_data, "Training Loss", "Training Loss", 
                      os.path.join(output_path, "loss_train_comparison_all_widths.png"))
